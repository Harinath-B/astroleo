...
class RouteManager:
    def __init__(self, node):
        self.node = node
        self.image_chunks = {}  # Temporary storage for image chunks per sender
    ...
    def receive_packet(self, serialized_packet):
        """
        Receive and process packets, handling chunked image data if applicable.
        """
        if not self.node.is_active():
            ...
            try:
                ...
                if len(self.image_chunks[sender_id]) == total_chunks:
                    ...
                    try:
                        ...
                        del self.image_chunks[sender_id]  # Clean up stored chunks
            except Exception as e:
